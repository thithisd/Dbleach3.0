...
function bindKeys()
 ...
 -- g_keyboard.bindKeyDown('Escape', function() finishAutoAttack() end, gameRootPanel)
...
function dash_loop()
  ...
  if g_keyboard.isKeyPressed('w') then
   g_game.use(g_map.getTile({x = localPosition.x, y = localPosition.y - 6, z = localPosition.z}):getTopThing())

  elseif g_keyboard.isKeyPressed('e') then
   g_game.use(g_map.getTile({x = localPosition.x + 3, y = localPosition.y - 3, z = localPosition.z}):getTopThing())

  elseif g_keyboard.isKeyPressed('d') then
   g_game.use(g_map.getTile({x = localPosition.x + 7, y = localPosition.y, z = localPosition.z}):getTopThing())
...
function show()
  ...
  if modules.client_options.getOption('displayExec') then
    g_window.setTitle(g_game.getCharacterName())
  end
  addEvent(function()
  ...
  -- classic control
    ...
    if useThing and keyboardModifiers == KeyboardNoModifier and mouseButton == MouseRightButton and not g_mouse.isPressed(MouseLeftButton) then
      ...
      if attackCreature and attackCreature ~= player then
        if modules.game_battle.autoAttackTarget:isChecked() then
          g_game.attack(attackCreature)
          modules.game_battle.doAttackTarget()
        else
          g_game.attack(attackCreature)
        end
        return true
