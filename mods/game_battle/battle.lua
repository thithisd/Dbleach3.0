...
function init()
  ...
  g_keyboard.bindKeyDown('Escape', finishAutoAttack)
  g_keyboard.bindKeyDown('Ctrl+X', autoAttackTargetActions)
  ...
  autoAttackTarget = battleWindow:recursiveGetChildById('autoAttackTarget')
...
function finishAutoAttack()
  g_game.cancelAttackAndFollow()
  g_game.cancelAttack()
  removeEvent(C_keepAttack) 
  C_keepAttackLast = nil

end

function autoAttackTargetActions()
    if autoAttackTarget:isChecked() == false then
      autoAttackTarget:setChecked(true)
      if g_game.isAttacking() then
        doAttackTarget()
      end
    else
      autoAttackTarget:setChecked(false)
      g_game.cancelAttack()
      removeEvent(C_keepAttack) 
      C_keepAttackLast = nil
    end
...
function doCreatureFitFilters(creature)
  ...
  local autoAttackTarget = autoAttackTarget:isChecked()
...
-- Auto Attack Target by Ricca, não alterar valores pois podem danificar seu cliente.
function doAttackTarget()
  removeEvent(C_keepAttack) 
  C_keepAttackLast = nil
  local player = g_game.getLocalPlayer()
  local antispam = g_clock.millis()
  C_keepAttack = cycleEvent(function() 

  if g_game.isOnline then
      if player:hasState(PlayerStates.Pz) then 
        return
      end
      if C_keepAttackLast and g_game.isAttacking() then
        return
      elseif C_keepAttackLast and not g_game.isAttacking() and g_clock.millis() - antispam > 300 then
        g_game.attack(C_keepAttackLast)
        antispam = g_clock.millis()
        return
      end
    if not C_keepAttackLast and g_game.isAttacking() then
      C_keepAttackLast = g_game.getAttackingCreature() 
    end
  end 
  end, 100)
end
  ...
  elseif mouseButton == MouseLeftButton and not g_mouse.isPressed(MouseRightButton) then
    ...
    if self.isTarget then
        finishAutoAttack()
  		...
  		if autoAttackTarget:isChecked() then    
        doAttackTarget()
  		end
    end
    return true
